...
noPts = 9;      % number of sampling points (pick odd)
...
%
% We want to add periodic bc's

% We add periodic bc's
A(1,1:2) = [4 1];
A(1, L+1)  = 1; 
A(L+1,L:L+1) = [1 4];
A(L+1,1)  = 1; 
...
plot(cpts(1,:), cpts(2,:), '-sr');
plot(midpts(1,:),midpts(2,:), '--+r');
